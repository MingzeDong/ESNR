...
class SinkhornKnopp:
    ...
    def fit(self, P):
        ...
        P_eps = np.copy(P)
        while np.any(np.sum(P_eps, axis=1) < min_threshr) \
        ...
        self._D1 = np.diag(np.squeeze(r))
        self._D2 = np.diag(np.squeeze(c))
        ### Dot product implementation was used for generating the results in the paper. 
        ### Substituted with the much faster element-wise multiplication implementation
        #P_eps = self._D1.dot(P).dot(self._D2)
